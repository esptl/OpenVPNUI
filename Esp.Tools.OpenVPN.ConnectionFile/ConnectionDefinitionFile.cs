...
namespace Esp.Tools.OpenVPN.ConnectionFile
{
    ...
    public class ConnectionDefinitionFile
    {
        ...
        public string ConfigurationData
        {
            get
            {
                return BuildConfigString(_configLines);
            }
            set
            {
                var lines = value.Split('\n', '\r').Select(pX=>pX.Trim('\n','\r','\t',' ')).Where(pX=>!string.IsNullOrEmpty(pX));
        ...
        private static readonly DataContractSerializer Serializer = new DataContractSerializer(typeof(ConnectionDefinitionFile));
